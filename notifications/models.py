...
class NotificationLog(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    notification_type = models.CharField(
        max_length=50, choices=NotificationType.choices, verbose_name=_('Notification Type')
    )
    member = models.ForeignKey(
        Member, on_delete=models.CASCADE, related_name='notification_logs', verbose_name=_('Member')
    )
...
class ExpiryNotificationQueue(models.Model):
    ...
    def __str__(self):
        return (
            f"Notification for {self.subscription} ({self.days_before_expiry} days before expiry)"
        )
