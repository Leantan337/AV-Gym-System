...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        notifications = ExpiryNotificationQueue.objects.filter(
            scheduled_date=process_date, is_processed=False
        )
        ...
        for notification in notifications:
            ...
            if subscription.status != 'active':
                self.stdout.write(
                    f"Skipping notification for inactive subscription: {subscription}"
                )
            ...
            if not options['dry_run']:
                ...
                try:
                    log = NotificationService.send_membership_expiry_notification(
                        subscription=subscription, days_before_expiry=days
                    )

                    self.stdout.write(
                        self.style.SUCCESS(f"Notification sent to {member.full_name}")
                        if log.is_email_sent
                        else self.style.WARNING(
                            f"Notification logged but email not sent to {member.full_name}"
                        )
                    )
                    ...
                    self.stderr.write(self.style.ERROR(f"Error sending notification: {str(e)}"))
            else:
                self.stdout.write(
