        ...
        for notification in due_notifications:
            try:
                with transaction.atomic():
                    ...
                    processed_count += 1
                    logger.info(f"Processed expiry notification for {notification.subscription.member.full_name}")
                    
            except Exception as e:
                logger.error(f"Error processing notification for {notification.subscription.member.full_name}: {e}")
                continue
