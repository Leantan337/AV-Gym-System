        ...
        for subscription in due_subscriptions:
            try:
                with transaction.atomic():
                    ...
                    if existing_invoice:
                        logger.info(f"Invoice already exists for member {subscription.member.full_name}")
                        continue
                    ...
                    generated_count += 1
                    logger.info(f"Generated invoice {invoice.number} for {subscription.member.full_name}")
                    
            except Exception as e:
                logger.error(f"Error generating invoice for member {subscription.member.full_name}: {e}")
                continue
