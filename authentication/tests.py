...
class RBACTestCase(APITestCase):
    def setUp(self):
        ...
        self.invoice_template = InvoiceTemplate.objects.create(
            name='Test Template',
            content='<html><body>Test Invoice Template</body></html>'
        )
    ...
    def test_admin_access(self):
        ...
        try:
            response = self.client.post(f"{reverse('invoice-detail', args=[self.invoice.id])}/mark_paid/")
            self.assertEqual(response.status_code, status.HTTP_200_OK)
        except:
    ...
    def test_staff_access(self):
        ...
        # Test invoice settings (should be forbidden)
        try:
            response = self.client.post('/api/admin/invoice-settings/', {
                'template_id': 1
            })
    ...
    def test_trainer_access(self):
        ...
        response = self.client.post(reverse('checkin-list'), {
...
class RBACTestCaseWithJWT(APITestCase):
    def setUp(self):
        ...
        self.member = Member.objects.create(
            user=self.member_user,
            full_name='Test Member JWT',
            membership_number='M101'
        )
        
        self.other_member = Member.objects.create(
            user=self.other_member_user,
            full_name='Other Member JWT',
            membership_number='M102'
        )
        ...
        # Create invoice template for tests
        self.invoice_template = InvoiceTemplate.objects.create(
            name='JWT Test Template',
            content='<html><body>JWT Test Invoice Template</body></html>'
        )
