...
class APITestSuite:
    ...
    def test_authentication_flow(self):
        ...
        try:
            ...
            if response.status_code in [201, 400]:  # 400 if user already exists
                ...
                self.log_test("User Registration", False, f"Unexpected status: {response.status_code}")
        except Exception as e:
        ...
        if self.auth_tokens.get("refresh"):
            try:
                response = self.session.post(f"{API_BASE}/auth/refresh/", 
                                           json={"refresh": self.auth_tokens["refresh"]})
                if response.status_code == 200:
                    data = response.json()
                    if "access" in data:
                        self.auth_tokens["access"] = data["access"]
    ...
    def test_password_reset_flow(self):
        ...
        try:
            ...
            if response.status_code in [200, 400]:  # 400 if email doesn't exist
                ...
                self.log_test("Password Reset Request", False, f"Unexpected status: {response.status_code}")
        except Exception as e:
    ...
    def test_member_management(self):
        ...
        if self.test_data.get("member_id"):
            try:
                response = self.session.get(f"{API_BASE}/members/{self.test_data['member_id']}/", headers=headers)
                if response.status_code == 200:
                    self.log_test("Member Retrieval", True, "Member retrieved successfully")
                else:
                    self.log_test("Member Retrieval", False, f"Retrieval failed: {response.status_code}")
            except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test("Member List", True, f"Retrieved {len(data.get('results', []))} members")
            else:
    ...
    def test_checkin_management(self):
        ...
        if self.test_data.get("member_id"):
            ...
            try:
                response = self.session.post(f"{API_BASE}/checkins/", json=checkin_data, headers=headers)
                if response.status_code == 201:
                    data = response.json()
                    self.test_data["checkin_id"] = data["id"]
                    self.log_test("Check-in Creation", True, f"Check-in created with ID: {data['id']}")
                else:
                    self.log_test("Check-in Creation", False, f"Creation failed: {response.status_code}")
            except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test("Check-in List", True, f"Retrieved {len(data.get('results', []))} check-ins")
            else:
    ...
    def test_invoice_management(self):
        ...
        try:
            response = self.session.post(f"{API_BASE}/invoices/", json=invoice_data, headers=headers)
            if response.status_code == 201:
                data = response.json()
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test("Invoice List", True, f"Retrieved {len(data.get('results', []))} invoices")
            else:
    ...
    def test_notification_management(self):
        ...
        try:
            response = self.session.post(f"{API_BASE}/notifications/", json=notification_data, headers=headers)
            if response.status_code == 201:
                data = response.json()
                self.test_data["notification_id"] = data["id"]
                self.log_test("Notification Creation", True, f"Notification created with ID: {data['id']}")
            else:
                self.log_test("Notification Creation", False, f"Creation failed: {response.status_code}")
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test("Notification List", True, f"Retrieved {len(data.get('results', []))} notifications")
            else:
    ...
    def test_report_management(self):
        ...
        try:
            response = self.session.post(f"{API_BASE}/reports/generate/", json=report_data, headers=headers)
            if response.status_code in [200, 201, 202]:  # Various success statuses
                ...
                self.log_test("Report Generation", False, f"Generation failed: {response.status_code}")
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test("Report List", True, f"Retrieved {len(data.get('results', []))} reports")
            else:
    ...
    def test_error_handling(self):
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                self.log_test("Invalid Authentication", False, f"Expected 401, got {response.status_code}")
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                self.log_test("Missing Authentication", False, f"Expected 401, got {response.status_code}")
        except Exception as e:
        ...
        if self.auth_tokens.get("access"):
            ...
            try:
                response = self.session.post(f"{API_BASE}/members/", json=invalid_data, headers=headers)
                if response.status_code == 400:
                    self.log_test("Invalid Data", True, "Properly rejected invalid data")
                else:
                    self.log_test("Invalid Data", False, f"Expected 400, got {response.status_code}")
            except Exception as e:
...
def main():
    ...
    try:
        ...
        if response.status_code != 200:
            print("❌ Server is not responding properly. Please ensure the Django server is running.")
