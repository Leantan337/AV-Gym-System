...
response = requests.post(
    f'{BASE_URL}/token/', {'username': 'gymadmin', 'password': '@12345678'}  # The correct password
)
...
if response.status_code == 200:
    ...
    if response.status_code == 201:
        ...
        if response.status_code == 201:
            ...
            response = requests.post(
                f'{BASE_URL}/subscriptions/', headers=headers, json=subscription_data
            )
            ...
            if response.status_code == 201:
                ...
                checkin_data = {'member': member_id}

                response = requests.post(
                    f'{BASE_URL}/checkins/', headers=headers, json=checkin_data
                )
