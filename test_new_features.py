...
# Get JWT token
response = requests.post(f'{BASE_URL}/token/', {'username': 'gymadmin', 'password': '@12345678'})
...
if response.status_code == 200:
    ...
    if response.status_code == 200 and response.json():
        ...
        response = requests.get(f'{BASE_URL}/invoices/{invoice_id}/generate_pdf/', headers=headers)
    ...
    if response.status_code == 200 and response.json():
        ...
        response = requests.post(
            ...
            headers=headers,
            json={'member_ids': member_ids, 'status': 'inactive'},
        )
