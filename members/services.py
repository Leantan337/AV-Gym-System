...
class IDCardGenerator:
    ...
    def generate_id_card_pdf(member):
        ...
        try:
            ...
            p = canvas.Canvas(
                buffer, pagesize=(216, 136)
            )  # 3.6x2.1 inches (86x54mm standard ID card size)
            ...
            try:
                if (
                    member.image
                    and hasattr(member.image, 'path')
                    and os.path.exists(member.image.path)
                ):
                    img = PILImage.open(member.image.path)
                else:
                    img = PILImage.open(
                        os.path.join(settings.STATIC_ROOT, 'img', 'placeholder.png')
                    )
            ...
            p.drawCentredString(
                width / 2,
                5,
                "IF FOUND, PLEASE RETURN TO AV GYM • 123 GYM STREET, WORKOUT CITY • (123) 456-7890",
            )
